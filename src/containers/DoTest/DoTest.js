...
export class DoTest extends Component {
  ...
  checkAnswer = (question_id, answer_id) => {
    let question = this.props.questionsAnswers.find(
  ...
  componentDidMount = () => {
    if (this.props.auth.user_id) {
      this.props.loadUserTestDetails(
        this.props.auth.user_id,
        this.props.loadUserTestResults,
        this.setLoading
      );
    }
  };
  ...
  findMarks = (questionAnswers, userAnswers) => {
    ...
    let questionCorrectAnswers = questionAnswers.filter(
      (itm) => itm.status === 1
    );
    return userCorrectAnswers.length - userInCorrectAnswers.length < 0
      ? 0
      : ((userCorrectAnswers.length - userInCorrectAnswers.length) * 100) /
          questionCorrectAnswers.length;
  };

  findTotalMarks = (questions) => {
    let total = 0;
    for (let question of questions) {
      total += this.findMarks(
        question.question.allQuestionAnswers,
    ...
    return (
      <Fragment>
        <div className="admin-containe">
          ...
          <Navbar />
          <div className="row">
            {(this.state.submitAnswer === true ||
              this.state.loading === true ||
              this.props.testResults === null) &&
            this.props.questionsAnswers !== "none" ? (
              <div
                className="col xl12 l12 m12 s12 white"
                      ...
                      this.props.testResults.length === 0 ? (
                        <div className="col xl6 l6 m6 s12">
                          <button
                            ...
                            className="waves-effect waves-light right my-btn bg-color hoverable main-btn"
                          >
                            Submit test
                          </button>
                        </div>
                    ...
                    {this.state.loading === true ? (
                      ...
                      <div>
                        ...
                        >
                          ...
                          {preparedTest.questions.length > 0 &&
                            preparedTest.questions.map((item, i) => (
                                ...
                                <div className="question-answers">
                                  {item.answers.map((itm, j) => (
                                    <div
                                      key={j + 1}
                                      onClick={() => {
                                        this.props.addUserQuestionAnswer(
                                          itm.question_id,
                                          itm.answer_id
                                        );
                                      }}
                        ...
                        <div
                          className="row test-footer"
                          style={{ margin: "0px" }}
                        >
                          <div className="col xl6 l6 m6 s12">
                            <Link
                              to="/topics"
                              className="waves-effect waves-light my-btn bg-color hoverable main-btn"
                      ...
                      this.props.testResults.length > 0 && (
                        <div>
                          ...
                          >
                            ...
                            preparedTest === undefined ? (
                              <Loading />
                            ) : preparedTest.show_correct_answer === 0 ? (
                              <div>
                                <center></center>
                              </div>
                            ) : (
                                ...
                                <div key={i + 1} className="question">
                                  ...
                                  >
                                    <div className="col s10 m10 l10 xl10">
                                      ...
                                      <div
                                        dangerouslySetInnerHTML={{
                                          __html: item.question.description,
                                        }}
                                        style={{
                                          marginLeft: "10px",
                                          marginTop: "10px",
                                    ...
                                    <div className="col s2 m2 l2 xl2 right">
                                      ...
                                      <div
                                        style={{
                                          ...
                                          color:
                                            this.findMarks(
                                              item.question.allQuestionAnswers,
                                              item.userAnswers
                                            ) >= 50
                                              ? "green"
                                              : "red",
                                  ...
                                  <div className="question-answers">
                                    {item.question.allQuestionAnswers.map(
                                      (itm, j) => (
                                        <div
                                          key={j + 1}
                                          className={`question-answers-item ${
                                            item.userAnswers.find(
                                              (response) =>
                                                response.answer_id ===
                                                itm.answer_id
                                            )
                                        ...
                                        >
                                          ...
                                          >
                                            {item.userAnswers.find(
                                              ...
                                              <div className="col">
                                                {item.question.allQuestionAnswers.filter(
                                                  (x) => x.status === 1
                                                ).length > 1 ? (
                                                  <ImCheckboxChecked className="left-icon" />
                                                ) : (
